    ...
    while True:
        ...
        if int(choice) not in range(confignum+1):
            ...
            try:
                ...
                returnlist = []           
                #create returnlist again
                returnlist.extend(list(configtext[2][-5:-1])) #Read "Mode" value
                int(configtext[2][-5:-1])
                returnlist.append(configtext[3][-2]) #Read "Count" value
                int(configtext[3][-2])
                #Convert the value of "Task" into a dictionary
                dicttemp = configtext[4][7:][:-1].strip("{}") #Extract the dictionary,now a string
                temp1 = dicttemp.split("") #instead of ,
                temp2 = []
                for tp in temp1:
                    temp2.extend(tp.replace("'","").strip(" ").split("")) #delete , and :
                temp3 = []
                for tp in temp2[:]:
                    temp3.append(tp.strip(" ")) #delete " "
                    #Even number index is Key, odd number index is Value
                lkeys = []
                lvalues = []
                for lkey,lvalue in enumerate(temp3): #Separate Keys and Values
                    if lkey % 2 == 0:
                        lkeys.append(lvalue)
                    else:
                        lvalues.append(lvalue)
                returndict = dict(zip(lkeys,lvalues)) #Convert to dictionary
                returnlist.append(returndict) #append to returnlist
